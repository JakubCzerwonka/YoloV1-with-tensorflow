...
def _iou_xywh(bboxes1, bboxes2):
    """Counts IoU"""
    bx1 = _xywh_to_x1y1x2y2(bboxes1)
    bx2 = _xywh_to_x1y1x2y2(bboxes2)

    b1_x1, b1_y1, b1_x2, b1_y2 = tf.split(bx1, 4, axis=-1)
    b2_x1, b2_y1, b2_x2, b2_y2 = tf.split(bx2, 4, axis=-1)
...
@register_keras_serializable(package="Custom", name="YoloLoss")
class YoloLoss(tf.keras.losses.Loss):
    """Yolo loss (assuming structure like: (batch, S, S, B * 5 + C)), where [b1_x, b1_y, b1_w, b1_h, b1_conf, b2_x, b2_y, b2_w, b2_h, b2_conf, classes/probs]"""
    def __init__(self, S=7, B=2, C=20, lambda_coord=5.0, lambda_noobj=0.5, reduction=tf.keras.losses.Reduction.SUM_OVER_BATCH_SIZE, name="yolo_loss"):
    ...
    def call(self, y_true, y_pred):
        ...
        pred_boxes = tf.stack(pred_boxes_list, axis=-2)                     

        pred_coords = pred_boxes[..., 0:4]                                  
        pred_confs  = pred_boxes[..., 4:5]                                  

        true_box = y_true[..., 0:4]                                       
        true_box_exp = tf.expand_dims(true_box, axis=-2)                    

        # Best box by IoU
        ious = _iou_xywh(true_box_exp, pred_coords)                        
        resp_idx = tf.argmax(ious, axis=-1, output_type=tf.int32)           
        resp_onehot = tf.one_hot(resp_idx, depth=self.B, dtype=tf.float32) 
        resp_onehot = tf.expand_dims(resp_onehot, axis=-1)                 
        chosen_box = tf.reduce_sum(pred_boxes * resp_onehot, axis=-2)    

        # If object and if not object mask
        obj_mask = tf.expand_dims(y_true[..., 4], axis=-1)                
        obj_mask_float = tf.cast(obj_mask, tf.float32)
        obj_mask_expanded = tf.expand_dims(obj_mask_float, axis=-2)        
        ...
        nonresp_in_obj_mask = obj_mask_expanded * (1.0 - resp_onehot)  
        ...
        class_loss = tf.square(class_true - class_pred)
